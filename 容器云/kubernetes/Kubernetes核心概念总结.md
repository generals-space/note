...
关于多重升级，举例，当你创建了一个`nginx1.7`的Deployment，要求副本数量为5之后，`Deployment Controller`会逐步的将5个1.7的Pod启动起来；当启动到3个的时候，你又发出更新`Deployment`中Nginx到1.9的命令；这时`Deployment Controller`会立即将已启动的3个1.7Pod杀掉，然后逐步启动1.9的Pod。Deployment Controller不会等到1.7的Pod都启动完成之后，再依次杀掉1.7，启动1.9。
...
### 10.2 Horizontal Pod Autoscaler的决策策略
